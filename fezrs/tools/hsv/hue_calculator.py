...
class HUECalculator(BaseTool):
    ...
    def __init__(self, nir_path: PathLike, blue_path: PathLike, green_path: PathLike):
        ...
        self.__hue_output: np.ndarray | None = None
    ...
    def calculate(self):
        ...
        self.__hue_output = rgb2hsv(np.dstack((nir, green, blue)))
        self.__hue_output = self.__hue_output[:,:,0]
        return self.__hue_output
    ...
    def export_file(self,
            ...
            filename_prefix: str = "HUE_output",
            dpi: int = 500,
            grid: bool = True
        ):
        
        if self.__hue_output is None:
            raise ValueError("HUE data not computed. Run `calculate()` before exporting.")
        ...
        im = ax.imshow(self.__hue_output, cmap=colormap)
...
if __name__ == "__main__":
    ...
    calculator = HUECalculator(blue_path=blue_path, green_path=green_path, nir_path=nir_path)
    calculator.run(output_path="./", show_axis=False)
